...
class Zarinpal extends Payment
{
	...
	public function callback()
	{
		...
		if($error = $client->getError())
            ...
            $au=$this->trans->transAu;
	        $userdata = Database::queryBuilder()
	            ->select('save.*')
	            ->from('save')
                ->join('trans ON save.saveId=trans.transSaveId','INNER JOIN')
	            ->where("trans.transAu = :au ")
	            ->getRow([':au' => $au]); 
            $userName=$userdata->saveName;
            $userEmail=$userdata->saveEmail;
            $userMobile=$userdata->saveMobile;
